...
class Env(object):
    ...
    def transform(self, info):
        world_size = 12
        state = np.zeros((world_size, world_size))
        
        player1 = info["player1"]
        player2 = info["player2"]

#        state[player1['x'], player1['y']] = 0
#        state[player1['home_x'], player1['home_y']] = 0
#        state[player2['x'], player2['y']] = 0
        state[player2['home_x'], player2['home_y']] = -1
        ...
        jobs = info["jobs"]
        for job in jobs:
            state[job["x"], job["y"]] = job["value"]

        return (player1['x'], player1['y']), (player2['x'], player2['y']), state
    ...
    def reset(self):
        ...
        self.current_player = self.player1
        info = self.get_state()
        (x1, y1), (x2, y2), state = self.transform(info)
...
if __name__ == '__main__':
    ...
    env = Env("","p1","p2",conf,random.Random(30))
    (x1, y1), (x2, y2), state = env.reset()
    env.render()
    
    print("****************", state)
    map2d = get_map2d(state)

    big_list = get_all_paths(map2d)
    while True:
        # if p1_actions:
...
#        print(state)
#        
        action1 = CNN_path(state, x1, y1, env.player1.n_jobs, player, big_list,map2d,env.steps/2)
        print(time.time() - start)
        print ('P1:',action1)
        print(env.steps)
        (x1, y1), (x2, y2), state = env.step("p1",action1)
#        print("next_state")
#        print(state)
        env.render()
        action2 = "S"
#        print ('P2:',action2)
        (x1, y1), (x2, y2), state = env.step("p2",action2)
#        env.render()
